/*
 * ============================================================================
 * hannote-data-align —— PowerJob 10 个任务「一键导入」浏览器脚本
 * ----------------------------------------------------------------------------
 ...
 *       循环调用 /job/save，一次性把 10 个任务全部创建出来。
 *
 * 用法：
 ...
 *   4. 看输出：每个任务打印「✅ 已创建」或「❌ 失败」。刷新任务管理页即可看到 10 个任务。
 *
 * 说明：
 *   - appId / PowerJwt 自动从 localStorage 读取（Power_appId / PowerJwt），
 *     与控制台自身请求用的是同一套凭据，等价于你手动点 10 次「导入」。
...
(async () => {
  ...
  // 8 个对齐任务（MapReduce）+ 2 个建/删表任务（单机），处理器全限定类名见 hannote-data-align。
  const ALIGN = [
    ...
    ['数据对齐-笔记评论数', 'NoteCommentCountAlignProcessor',  '按分片重算 t_note_count.comment_total 并同步 Redis'],
  ];
  ...
  const jobs = [
    {
      ...base,
      jobName: '数据对齐-创建日增量表',
      jobDescription: '每日创建次日的 8×shards 张临时表',
